...
<table id="artist" border="1" style="position: absolute; margin-top: 10%">
    <tr>
        ...
        if ($_SESSION['user_type'] == 'admin' or $_SESSION['user_type'] == 'org') {
            # code...
            echo '<th style="text-align: center" class="th_action">Action</th>';
        }
    ...
    <?php
        ...
        if($result){
            while ($row1 = mysqli_fetch_array($result)) {
                ...
                echo '<td style="text-align: center"><img src="'.$row1["IMG"].'" width="50px" class = "" ></td>';
                echo '<td class="nic" name="nic">'.$row1["NIC"].'</td>';
                echo '<td class="name" name="name">'.$row1["NAME"].'</td>';
                 echo '<td class="name" name="name">'.$row1["ADDRESS"].'</td>';
