...
if(isset($_POST['reg_user'])){
    ...
    if (empty($email_error) && empty($pass_error)) {
        ...
        $send_to_db = "INSERT INTO `users` (`NAME`, `NIC`, `GENDER`, `MOBILE`, `EMAIL`, `PASSWORD`, `ADDRESS`, `IMG`, `USER_TYPE`) VALUES ('{$name}', '{$nic}', '{$gender}', '{$phone}', '{$email}', '{$hashed_pw}', '', '', '{$user_type}')";
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="CSS/Login/Jud&OrgLoginStyle.css">
    <link rel="stylesheet" type="text/css" href="CSS/Register/Jud&OrgRegFormStyle.css">
...
<form name="myForm" action="<?php echo $_SERVER["PHP_SELF"]; ?>" onsubmit="return validateForm()" method="post" style="margin-top: 6%" required>
    <div class="container">
        <h1>Register Form</h1>
        <p class="ctgName">Judge Board and Organization Team</p>
        <hr>
        ...
        <h2 style="font-size: 18px">Select user type</h2>

        <table>
            <tr>
                <td>
                    <label style="margin-top: 1%; margin-right: 50px" class="select">Judge
                        <input type="radio" name="user_type" value="Judge">
                        <span class="checkmark"></span>
                    </label>
                </td>
                <td>
                    <label style="margin-top: 1%; margin-right: 50px" class="select">Org. Team
                        <input type="radio" name="user_type" value="Org">
                        <span class="checkmark"></span>
                    </label>
                </td>
            </tr>
